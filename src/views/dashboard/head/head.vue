<template>
  <div class="head-block dashboard-head">
    <div class="left-block head-centent">
      <div @click="changCollapse()" :class="{ 'expand-block': menuCollapse.value }">
        <my-icon :iconType="'Expand'"></my-icon>
      </div>
      <!-- 面包屑 -->
      <div>
            ...
            <el-popover v-if="item.hasChildren" placement="bottom" :width="'auto'" trigger="click">
              <template #reference>
                <div class="breadcrumb-title-block breadcrumb-font">
                  ...
                  <my-icon :icon-type="'ArrowDown'" :size="'12px'"></my-icon>
                </div>
              </template>
    ...
    <div class="right-block head-centent">
      <!-- 搜索模块 -->
      <div>
        <my-icon :icon-type="'Search'"></my-icon>
      </div>
      <!-- 报警模块 -->
      <el-popover placement="bottom" :width="'auto'" trigger="click">
        <template #reference>
          <div>
            <my-icon :icon-type="'BellFilled'"></my-icon>
          </div>
        </template>
        <!-- 消息模块 -->
      ...
      <!-- 最大化 -->
      <div>
        <my-icon :icon-type="'FullScreen'"></my-icon>
      </div>
      <!-- 用户信息模块 -->
      <el-popover placement="bottom" :width="'auto'" trigger="hover">
        <template #reference>
          <div class="user-block">
            <my-icon :iconType="'Avatar'"></my-icon>
            <p>admin</p>
          </div>
        </template>
        <div class="head-router-select" @click="logOut()">
          <my-icon :icon-type="'SwitchButton'"></my-icon><span>退出系统</span></div
        >
      </el-popover>
      <!-- 设置模块 -->
      <div @click="globalClick">
        <my-icon :icon-type="'Tools'"></my-icon>
