<template>
  <div class="head-block dashboard-head">
    <div class="left-block head-centent">
      <div
        @click="changCollapse()"
        :class="{ 'expand-block': menuCollapse.value }"
      >
        <my-icon :iconType="'Expand'" :color="iconClolor"></my-icon>
      </div>
      <!-- 面包屑 -->
      <div>
    ...
    <div class="right-block head-centent">
      ...
      <el-popover placement="bottom" :width="'auto'" trigger="hover">
        <template #reference>
          <div class="user-block">
            <my-icon :iconType="'Avatar'" :color="iconClolor"></my-icon>
            <p>admin</p>
          </div>
        </template>
        <div class="head-router-select" @click="logOut()">
          <my-icon :icon-type="'SwitchButton'" :color="iconClolor"></my-icon
          ><span>退出系统</span>
        </div>
      </el-popover>
      <!-- 设置模块 -->
      <div @click="globalClick">
        ...
        <global-settings
          v-model:globalSettingDrawer="globalSettingDrawer"
        ></global-settings>
      </div>
    </div>
  </div>
